...
func newFinancialTestPool(t *testing.T) *pgxpool.Pool {
	...
	if _, err := pool.Exec(ctx, `
		TRUNCATE TABLE
			...
			card.statements,
			card.auto_sell_executions,
			card.disputes,
			card.refunds,
			card.reversals,
			card.captures,
			card.authorizations,
			card.provider_events,
			card.auto_sell_mandate_assets,
			card.auto_sell_mandates,
			card.collateral_accounts,
			card.command_requests,
			card.lifecycle_events,
			card.cards,
			card.customer_profiles,
			crypto.reconciliation_runs,
			crypto.provider_events,
			crypto.withdrawal_requests,
			crypto.withdrawals,
			crypto.withdrawal_address_events,
			crypto.withdrawal_address_requests,
			crypto.withdrawal_addresses,
			crypto.deposits,
			crypto.deposit_address_requests,
			crypto.deposit_addresses,
			crypto.fills,
			crypto.child_orders,
			crypto.legs,
			crypto.conversion_requests,
			crypto.conversions,
			crypto.asset_ledger_accounts,
			crypto.customer_profiles,
			...
			securities.instrument_ledger_accounts,
			securities.orders,
			securities.paper_accounts,
